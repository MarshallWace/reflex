...
impl MultiplyNode {
    pub fn new(left: Rc<Expression>, right: Rc<Expression>) -> MultiplyNode {
...
impl CompoundNode for MultiplyNode {
    fn factory(args: Vec<Rc<Expression>>) -> NodeFactoryResult {
        ...
        NodeFactoryResult::Ok(Node::Multiply(MultiplyNode::new(left, right)))
    }
    fn expressions(&self) -> Vec<&Rc<Expression>> {
        vec![&self.left, &self.right]
    }
}
impl Evaluate for MultiplyNode {
...
mod tests {
    ...
    fn multiplication_expressions() {
        let env = Env::new();
        let expression = parser::parse("(multiply 0 0)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(*result, Expression::Value(Value::Int(0 * 0)));
        let expression = parser::parse("(multiply 3 4)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        ...
        let expression = parser::parse("(multiply 0.0 0.0)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(*result, Expression::Value(Value::Float(0.0 * 0.0)));
        let expression = parser::parse("(multiply 2.718 3.142)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
    ...
    fn invalid_multiplication_expression_operands() {
        let env = Env::new();
        let expression = parser::parse("(multiply 3 3.142)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(
            *result,
        ...
        let expression = parser::parse("(multiply 3.142 3)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(
            *result,
            Expression::Error(String::from(
        ...
        let expression = parser::parse("(multiply 3 null)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(
            *result,
            Expression::Error(String::from(
        ...
        let expression = parser::parse("(multiply 3 false)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(
            *result,
            Expression::Error(String::from(
        ...
        let expression = parser::parse("(multiply 3 \"3\")", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(
            *result,
            Expression::Error(String::from(
        ...
        let expression = parser::parse("(multiply null 3)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(
            *result,
            Expression::Error(String::from(
        ...
        let expression = parser::parse("(multiply false 3)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(
            *result,
            Expression::Error(String::from(
        ...
        let expression = parser::parse("(multiply 3.142 null)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(
            *result,
            Expression::Error(String::from(
        ...
        let expression = parser::parse("(multiply 3.142 false)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(
            *result,
            Expression::Error(String::from(
        ...
        let expression = parser::parse("(multiply null 3.142)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(
            *result,
            Expression::Error(String::from(
        ...
        let expression = parser::parse("(multiply false 3.142)", &Node::factory).unwrap();
        let result = expression.evaluate(&env);
        assert_eq!(
            *result,
            Expression::Error(String::from(
