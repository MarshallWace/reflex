    ...
    TMetricLabels: Fn(Option<&str>, &GraphQlOperationPayload) -> Vec<(String, String)>,
{
        ...
        TAction: Action + OutboundAction<QueryUnsubscribeAction<T>>,
    {
        ...
        let removed_entry = {
            let removed_entry_index = {
                let existing_entry = self.state.operations.iter_mut().enumerate().find_map(
                    |(entry_index, entry)| {
                        entry
                            .subscriptions
                            .iter()
                ...
                match existing_entry {
                    None => None,
                    Some((entry_index, subscription_index, existing_entry)) => {
                        let removed_subscription =
                            existing_entry.subscriptions.remove(subscription_index);
                        decrement_gauge!(
                            METRIC_GRAPHQL_ACTIVE_OPERATION_COUNT,
                            1.0,
                            &removed_subscription.metric_labels
                        );
                        if existing_entry.subscriptions.is_empty() {
                            Some(entry_index)
                        } else {
                            None
                        }
            ...
            removed_entry_index.map(|index| self.state.operations.remove(index))
        };
        if let Some(entry) = removed_entry {
            StateTransition::new(Some(StateOperation::Send(
                context.pid(),
                QueryUnsubscribeAction { query: entry.query }.into(),
            )))
        } else {
            StateTransition::new(None)
        }
            ...
            + OutboundAction<GraphQlServerUnsubscribeAction<T>>,
    {
        ...
        match updated_operation {
            None => StateTransition::new(None),
            Some((entry_index, subscription_index, updated_operation)) => {
                match reflex_graphql::parse_graphql_operation(
                    ...
                    Ok(query) => {
                        let existing_expression = &self.state.operations[entry_index].query;
                        if existing_expression.id() == query.id() {
                            StateTransition::new(None)
                        } else {
                            let existing_subscription = self.state.operations[entry_index]
                                .subscriptions
                            ...
                            if self.state.operations[entry_index].subscriptions.is_empty() {
                                self.state.operations.remove(entry_index);
                            }
                            if let Some(existing_entry) = self
                                .state
                                .operations
            ...
            + OutboundAction<GraphQlServerParseErrorAction<T>>,
    {
        ...
        if let Some((metric_labels, duration)) =
            self.state.operations.iter_mut().find_map(|operation| {
                operation
                    .subscriptions
                    .iter_mut()
