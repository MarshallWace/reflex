...
use crate::{get_root_operation, GraphQlText};
...
type QueryFragments<'src, 'a> =
    HashMap<&'a GraphQlText<'src>, &'a FragmentDefinition<'src, GraphQlText<'src>>>;
...
///
/// This helper is intended for use with the [inject_query_arguments] transform factory.
///
/// # Examples
///
/// ```
/// use reflex_graphql::inject_args::{parse_argument_template, inject_query_arguments};
///
...
/// ]).unwrap();
///
/// let transform = inject_query_arguments(injected_args);
/// ```
...
/// GraphQL transform factory that injects the given field arguments into a GraphQL query.
///
...
///
/// The factory `arguments` are typically generated by the [parse_argument_template] helper.
///
/// # Examples
///
/// ```
...
///
/// let transform = inject_query_arguments(injected_args);
pub fn inject_query_arguments(
    arguments: InjectedQueryArguments,
) -> impl for<'src> Fn(
    Document<'src, GraphQlText<'src>>,
) -> Result<Document<'src, GraphQlText<'src>>, String> {
    move |document| {
        inject_document_arguments(&document, &arguments).map(|transformed| match transformed {
...
fn inject_document_arguments<'src>(
    document: &Document<'src, GraphQlText<'src>>,
    arguments: &InjectedQueryArguments,
) -> Result<Option<Document<'src, GraphQlText<'src>>>, String> {
...
fn inject_operation_arguments<'src>(
    operation: &OperationDefinition<'src, GraphQlText<'src>>,
    arguments: &InjectedQueryArguments,
    fragments: &QueryFragments<'src, '_>,
) -> Option<OperationDefinition<'src, GraphQlText<'src>>> {
    match operation {
...
fn inject_selection_set_arguments<'src>(
    selection_set: &SelectionSet<'src, GraphQlText<'src>>,
    arguments: &NestedArgs,
    fragments: &QueryFragments<'src, '_>,
) -> Option<SelectionSet<'src, GraphQlText<'src>>> {
...
fn inject_selection_arguments<'src>(
    selection: &Selection<'src, GraphQlText<'src>>,
    arguments: &NestedArgs,
    fragments: &QueryFragments<'src, '_>,
) -> Option<Vec<Selection<'src, GraphQlText<'src>>>> {
    match selection {
...
fn inject_field_arguments<'src>(
    field: &Field<'src, GraphQlText<'src>>,
    arguments: &NestedArgs,
    fragments: &QueryFragments<'src, '_>,
) -> Option<Field<'src, GraphQlText<'src>>> {
    find_named_argument(arguments, &field.name).and_then(|(_, branch)| {
        ...
        match &branch.value {
            Some(injected_args) => {
                let injected_args = injected_args.iter().cloned().map(|(name, value)| {
                    (
                        GraphQlText::<'src>::Owned(String::from(name)),
...
fn inject_fragment_spread_arguments<'src>(
    fragment: &FragmentSpread<'src, GraphQlText<'src>>,
    arguments: &NestedArgs,
    fragments: &QueryFragments<'src, '_>,
) -> Option<Vec<Selection<'src, GraphQlText<'src>>>> {
...
fn instantiate_runtime_arg_value<'a>(value: &Value<'static, String>) -> Value<'a, GraphQlText<'a>> {
    match &value {
        Value::Variable(name) => Value::Variable(GraphQlText::<'a>::Owned(String::from(name))),
        Value::Int(value) => Value::Int(value.clone()),
        Value::Float(value) => Value::Float(*value),
        Value::String(value) => Value::String(value.clone()),
        Value::Boolean(value) => Value::Boolean(*value),
        Value::Null => Value::Null,
        Value::Enum(variant) => Value::Enum(GraphQlText::<'a>::Owned(String::from(variant))),
        Value::List(items) => {
        ...
        Value::Object(fields) => Value::Object(
            fields
                ...
                .map(|(key, value)| {
                    (
                        GraphQlText::<'a>::Owned(String::from(key)),
                        instantiate_runtime_arg_value(value),
                    )
                })
                .collect(),
...
mod tests {
    ...
    use super::{inject_query_arguments, parse_argument_template};
